      ...
      <form @submit.prevent="onUpdate" class="space-y-4">
        ...
        <div>
          ...
          <select v-model="transaction.category"
            ...
            <option v-for="category in categories.filter(c => c.type === transaction.type)" :key="category.id" :value="category.name">
              {{ category.name }}
            </option>
          </select>
        </div>
        <div>
          ...
          <div class="relative">
            ...
            <input v-model.number="transaction.amount" type="number" placeholder="0.00"
              min="0" step="0.01"
        ...
        <div>
          ...
          <textarea v-model="transaction.description" placeholder="Enter transaction description..."
            rows="3"
        ...
        <button type="submit" class="w-full py-2 bg-green-600 text-white rounded-lg font-semibold mt-2">Update Transaction</button>
      </form>
    </div>
  </div>
</template>
