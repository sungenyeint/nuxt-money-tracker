    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div class="flex gap-2">
                <button v-for="period in ['week', 'month', 'year', 'all']" :key="period"
                    @click="timePeriod = period as any" :class="[
                        'px-4 py-2 rounded-lg font-medium text-sm transition-colors',
                        timePeriod === period
                            ? 'bg-blue-600 text-white'
                            : 'dark:bg-gray-400 dark:text-white bg-gray-100 text-gray-700 hover:bg-gray-200',
                    ]">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <!-- Average Transaction -->
            <div class="bg-white dark:bg-gray-600 rounded-xl shadow p-5">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-600 dark:text-white mb-1">Avg Transaction</p>
                        <p class="text-2xl font-bold text-gray-800 dark:text-white">
                            {{ formatCurrency(avgTransaction.toFixed(2), userSettings.currency) }}
                        </p>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-600 rounded-xl shadow p-5">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-600 dark:text-white mb-1">Total Transactions</p>
                        <p class="text-2xl font-bold text-gray-800 dark:text-white">
            ...
            <div class="bg-white dark:bg-gray-600 rounded-xl shadow p-5">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-600 dark:text-white mb-1">Savings Rate</p>
                        <p :class="[
                            'text-2xl font-bold',
                    ...
                    <TrendingUp v-if="savingsRate >= 0" class="w-10 h-10 text-green-500 opacity-20" />
            ...
            <div class="bg-white dark:bg-gray-600 rounded-xl shadow p-5">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-600 dark:text-white mb-1">Largest Transaction</p>
                        <p v-if="largestTransaction" :class="[
                            'text-2xl font-bold',
                            largestTransaction.type === 'income'
                                ? 'text-green-600'
                                : 'text-red-600',
                        ]">
                            {{ formatCurrency(largestTransaction.amount.toFixed(2), userSettings.currency) }}
                        </p>
                        <p v-else class="text-2xl font-bold text-gray-400">N/A</p>
                        <p v-if="largestTransaction" class="text-xs text-gray-500 dark:text-white mt-1 truncate">
        ...
        <!-- Empty State -->
        <div v-if="filteredTransactions.length === 0" class="bg-white rounded-xl shadow p-12 text-center">
